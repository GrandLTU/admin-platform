...
const rootPath = upath.normalizeSafe(argv.rootPath || 'public/assets');
const adminRootPath = upath.joinSafe(rootPath, 'admin');
const vendorPath = upath.normalizeSafe(argv.vendorPath || 'vendor/sylius');
const vendorUiPath = upath.joinSafe(vendorPath, 'ui-bundle');
...
const paths = {
    admin: {
        js: [
            ...
            upath.normalizeSafe('private/js/**'),
        ],
        sass: [
        ...
        img: [
            ...
            upath.normalizeSafe('private/img/**'),
        ],
    },
};
...
const sourcePathMap = [
    {
        sourceDir: upath.relative('', upath.normalizeSafe('private/')),
        destPath: '/app',
    },
    {
...
export const buildAdminJs = async function buildAdminJs() {
    const bundle = await rollup({
        input: upath.normalizeSafe('private/js/app.js'),
        plugins: [
            {
                name: 'shim-app',

                transform(code, id) {
                    if (upath.relative('', id) === upath.relative('', upath.normalizeSafe('private/js/app.js'))) {
                        return {
                            code: dedent`
